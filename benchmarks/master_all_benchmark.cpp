...
void runBenchmarkRow(const string& testName, vector<u32>& original) {
    ...
    // Find best
    double best_time = min({t_std, t_pdq, t_spread, t_qi, t_hf, t_apex, t_par});
    string winner = "";
    if (best_time == t_par) winner = "apex (Parallel)";
    else if (best_time == t_hf) winner = "qi::hyperfield";
    else if (best_time == t_apex) winner = "qi::apex";
...
int main() {
    ...
    cout << "  Comparing: std::sort vs pdqsort vs spreadsort vs qi::sort vs qi::hyperfield v4.0 vs qi::apex\n";
    ...
    {
        vector<u32> data(N10M); for (size_t i = 0; i < N10M; ++i) data[i] = 1000000 + (rng() % 50000);
        runBenchmarkRow("5. Shifted 16-bit Window", data);
    }
    {
        vector<u32> data(N10M); for (size_t i = 0; i < N10M; ++i) data[i] = i;
        runBenchmarkRow("6. Already Sorted", data);
    }
    {
        vector<u32> data(N10M); for (size_t i = 0; i < N10M; ++i) data[i] = N10M - 1 - i;
        runBenchmarkRow("7. Reverse Sorted", data);
    }
    {
        vector<u32> data(N10M); for (size_t i = 0; i < N10M; ++i) data[i] = i;
        for (size_t i = 0; i < N10M / 50; ++i) data[rng() % N10M] = rng() % N10M;
        runBenchmarkRow("8. Nearly Sorted (~98%)", data);
    }
    {
        vector<u32> data(N10M); for (size_t i = 0; i < N10M; ++i) data[i] = i % 1000;
        runBenchmarkRow("9. Sawtooth (Period 1K)", data);
    }
    {
        ...
        runBenchmarkRow("10. Pipe Organ", data);
    }
