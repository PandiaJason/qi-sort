...
int main() {
    std::cout << "========================================================================================\n";
    std::cout << "  THE ULTIMATE ARENA: QI-BEAST vs QI::SORT vs STD::SORT\n";
    std::cout << "========================================================================================\n\n";
    ...
    for (size_t N : sizes) {
        ...
        // 3. QI-BEAST (Single-Core)
        {
            auto data = original;
            double t = time_ms([&]() {
                data = original;
                qi_beast::sort(data.data(), N);
            });
            bool ok = std::is_sorted(data.begin(), data.end());
            double mkeys = (N / 1e6) / (t / 1000.0);
            std::cout << std::left << std::setw(44) << "QI-BEAST (Single-Core)"
                      << std::setw(16) << std::fixed << std::setprecision(2) << t
                      << std::setw(24) << mkeys
                      << (ok ? "PASS" : "FAIL") << "\n";
        }

        // 4. QI-BEAST (Multi-Core Parallel)
        {
            auto data = original;
            double t = time_ms([&]() {
                data = original;
                qi_beast::parallel_sort(data.data(), N);
            });
            bool ok = std::is_sorted(data.begin(), data.end());
            double mkeys = (N / 1e6) / (t / 1000.0);
            std::cout << std::left << std::setw(44) << "QI-BEAST (Multi-Core PARALLEL)"
                      << std::setw(16) << std::fixed << std::setprecision(2) << t
                      << std::setw(24) << mkeys
                      << (ok ? "PASS" : "FAIL") << "\n";
        }
