...
// Include HyperField-X and predecessors
#include "qi_hyper_field_x.hpp"
...
void testN(size_t N, mt19937_64& rng) {
    ...
    // 3. QI-HyperField-X (Zero-Division 64K Lattice)
    auto d_x = original;
    double t_x = time_ms([&]() { d_x = original; qi::hyper_field_x::sort(d_x.data(), N); });
    assert(is_sorted(d_x.begin(), d_x.end()));
    cout << left << setw(44) << "3. QI-HyperField-X (Zero-Division 64K)"
         << setw(16) << fixed << setprecision(2) << t_x
         << setw(22) << setprecision(1) << (N / 1e6) / (t_x / 1000.0)
         << setprecision(2) << (t_std / t_x) << "x FASTER\n";
...
int main() {
    cout << "========================================================================================\n";
    cout << "  THE HYPERFIELD-X SPEED-OF-LIGHT RACE\n";
