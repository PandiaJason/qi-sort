...
Demonstrates how qi::sort accelerates real-time 3D Particle Physics Simulations:
  ...
  2. qi::sort Frame-by-Frame Sub-Millisecond Spatial Key Sorting
  3. Spatial Coherency & Contiguous Cache-Friendly Near-Neighbor Interaction
  4. Frame Time Benchmark: qi::sort vs std::sort in 60 FPS Physics Loop
===============================================================================
*/
...
int main() {
    ...
    std::vector<uint32_t> spatial_keys(NUM_PARTICLES);
    ...
    for (int frame = 0; frame < NUM_FRAMES; ++frame) {
        ...
        for (size_t i = 0; i < NUM_PARTICLES; ++i) {
            ...
            spatial_keys[i] = compute_spatial_key(particles[i].x, particles[i].y, particles[i].z);
        }

        // 2. std::sort Baseline
        auto keys_std = spatial_keys;
        auto t0 = std::chrono::high_resolution_clock::now();
        std::sort(keys_std.begin(), keys_std.end());
        auto t1 = std::chrono::high_resolution_clock::now();
        total_std_ms += std::chrono::duration<double, std::milli>(t1 - t0).count();

        // 3. qi::sort Adaptive Engine
        auto keys_qi = spatial_keys;
    ...
    double fps_std = 1000.0 / (avg_std_frame_ms + 2.0); // +2ms for integration/render
    double fps_qi = 1000.0 / (avg_qi_frame_ms + 2.0);

    std::cout << std::left << std::setw(28) << "Physics Sorting Method"
              << std::setw(20) << "Avg Frame Sort (ms)"
              << std::setw(20) << "Simulated FPS"
              << std::setw(15) << "Speedup" << "\n";
    std::cout << std::string(80, '-') << "\n";

    std::cout << std::left << std::setw(28) << "Standard std::sort"
              << std::setw(20) << (std::to_string(avg_std_frame_ms).substr(0, 5) + " ms")
              << std::setw(20) << (std::to_string(fps_std).substr(0, 5) + " FPS")
              << "1.00x\n";

    std::cout << std::left << std::setw(28) << "qi::sort Engine"
    ...
    std::cout << "\n=========================================================================\n";
    std::cout << "  SUCCESS: 250,000 Particle Physics Simulation running smoothly with qi::sort!\n";
    std::cout << "=========================================================================\n";
